...
export function publicDocument(doc) {
  ...
  return {
    ...
    files: files.map((file) => ({
      ...
      size: file.size || 0,
      uploadedAt: file.uploadedAt || doc.createdAt,
    })),
...
export async function deleteDocument(code) {
  ...
  const paths = [
    ...documentFiles(existing).map((file) => file.pdfPathname).filter(Boolean),
    `${DOCUMENT_PREFIX}${normalizedCode}.json`,
  ];
  await del(paths, { token: getBlobToken() });
}

export function documentFiles(doc) {
...
export function findDocumentFile(doc, fileId) {
  const files = documentFiles(doc);
